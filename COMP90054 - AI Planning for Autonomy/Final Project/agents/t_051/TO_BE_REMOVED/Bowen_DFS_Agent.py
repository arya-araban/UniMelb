...
class myAgent():
    ...
    def SelectAction(self, actions, rootstate):
        ...
        while len(queue) and time.time()-start_time < THINKTIME:
            state, path = queue.pop() # Pop the next node (state, path) in the queue.
            ...
            for a in new_actions: # Then, for each of these actions...
                ...
                if goal:
                    print(f'DFS Path found:', next_path)
    ...
    '''
def CheckFirstRowGridState(self, state):
        ...
        for c in cols:
            ...
            if (gridState[0][c]) == 1:
                ...
                def CheckCrossCentreGridState(self, state):
        goal_count = 0
        #two_goals = ["crossCentre","completeRow"]
        ...
        for r in cells:
            if goal_count == 1:
                break
            if (gridState[r][2]) == 1 and r != 2:
                goal_count += 1
                two_goals.remove("crossCentre")
                return True
        ...
        if len(two_goals) == 1:
            ...
            return state.agents[self.id].GetCompletedRows() == 1
