...
class BidirectionalFoodSearchProblem:
    ...
    def __init__(self, startingGameState):
        ...
        # the starting state will have position of pacman alongside the food grid
        self.start = (self.init_pos, self.foodGrid)

        # And if you have anything else want to initialize:
        self.num_food = self.foodGrid.count()
    ...
    def getGoalStates(self):
        ...
        food_grid = self.foodGrid.deepCopy()
        pacman_locs = self.foodGrid.asList()

        for locs in pacman_locs:
            food_grid[locs[0]][locs[1]] = 'e'  # e for eaten
            goal_states.append((locs, food_grid))
        return goal_states

    def isGoalState(self, state):
        """You code here for Task 3:"""
        goal_achieved = False
        cur_food_grid = state[1]

        if not cur_food_grid.asList():  # meaning there are no remaining food (or 'T's) in the food grid
            goal_achieved = True

        # checks whether in the 2d grid we only have False
    ...
    def getSuccessors(self, state):
        ...
        legal_neighbors = Actions.getLegalNeighbors(cur_position, self.walls)

        for direction in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:

            new_position = Actions.getSuccessor(cur_position, direction)

            if new_position not in legal_neighbors:
                continue

            cost = 1
            food_grid_cp = cur_food_grid

            if cur_food_grid[int(new_position[0])][int(new_position[1])] == True:
                food_grid_cp = cur_food_grid.deepCopy()
                food_grid_cp[int(new_position[0])][int(new_position[1])] = 'e'

            new_state = (new_position, food_grid_cp)
            # these next few lines of code check to see if pacman gets closer to the closest food. if so, lower cost.
            dst = math.inf
            closest_pos = (0, 0)
            for food_pos in cur_food_grid.asList():
                distance = util.manhattanDistance(food_pos, cur_position)
                if distance < dst:
                    dst = distance
                    closest_pos = food_pos
            if util.manhattanDistance(closest_pos, new_position) < util.manhattanDistance(closest_pos, cur_position):
                cost = 0.75
    ...
    def getBackwardsSuccessors(self, state):
        ...
        for direction in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if cur_food_grid[int(cur_position[0])][int(cur_position[1])] == 'e':
                food_grid_cp = cur_food_grid.deepCopy()
                food_grid_cp[int(cur_position[0])][int(cur_position[1])] = True

            new_state = (new_position, food_grid_cp)
            if util.manhattanDistance(cur_position, self.getStartState()[0]) > util.manhattanDistance(
                    new_position, self.getStartState()[0]):  # if getting closer to goal state, decrease cost
                cost = 0.75
            successors.append((new_state, direction, cost))
        # There are four actions might be available:
        # for direction in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
...
def bidirectionalFoodProblemHeuristic(state, problem):
    ...
    x = 0  # x is distance between two most far away foods
    if len(foodList) == 0:
        return 0

    if len(foodList) != 1:
        x = max(util.manhattanDistance(p1, p2)
                for i, p1 in enumerate(foodList)
                for p2 in foodList[i + 1:])

    # y is the distance between pacman and the food closest to it.
    y = min(util.manhattanDistance(position, p2) for p2 in foodList)

    # The heuristic value is the distance to the closest food
    return x + y
...
def bidirectionalFoodProblemBackwardsHeuristic(state, problem):
    ...
    distance_to_start = util.manhattanDistance(position, problem.getStartState()[0])
